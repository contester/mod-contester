...
/// Print the main part of the page
	...
	foreach ($solutions as $solution)
	{
		...
		$row = array();
		$user = get_record_sql("SELECT user.firstname, user.lastname FROM mdl_user as user, mdl_contester_submits as submit
			WHERE submit.id={$solution['id']} AND user.id = submit.student");
		$row[]= $user->firstname.' '.$user->lastname;
		$time = get_record_sql("SELECT MAX(res.timex) as time FROM mdl_contester_results as res 
			WHERE 
			res.testingid={$solution['id']}");
		$row[]= $time->time;
		$length = get_record_sql("SELECT CHAR_LENGTH(solution) as len from mdl_contester_submits
...
/*	$sql = "SELECT CONCAT( user.firstname, ' ', user.lastname ), MAX(results.timex), $size FROM
	mdl_user as user, mdl_contester_results as results, mdl_contester_submits as submits, mdl_contester_testings as testings, 
	mdl_contester_problemmap as map, mdl_contester_problems as problems WHERE
